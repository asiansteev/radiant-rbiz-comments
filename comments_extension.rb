...
class CommentsExtension < Radiant::Extension
  version "0.0.5"
  description "Adds blog-like comments and comment functionality to products."
  ...
  define_routes do |map|                
    map.with_options(:controller => 'admin/comments') do |comments| 
      ...
      comments.connect 'admin/products/:product_id/comments/:status.:format'
      comments.connect 'admin/products/:product_id/comments/all.:format'
      
      comments.resources :comments, :path_prefix => "/admin", :name_prefix => "admin_", :member => {:approve => :get, :unapprove => :get}
      comments.admin_product_comments 'admin/product/:product_id/comments/:action'
      comments.admin_product_comment 'admin/products/:product_id/comments/:id/:action'
    end

    # This needs to be last, otherwise it hoses the admin routes.
    map.resources :comments#, :name_prefix => "product_", :path_prefix => "*url", :controller => "comments"
  end
  ...
  def activate
    ...
    if admin.respond_to? :product
      admin.product.edit.add :parts_bottom, "edit_comments_enabled", :before => "edit_timestamp"
      admin.product.index.add :sitemap_head, "index_head_view_comments"
      admin.product.index.add :node, "index_view_comments"
    end
